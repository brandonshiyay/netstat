...
def run(inteval=30):
	...
	if len(conn_csv.read())==0:
		conn_csv.write('status,date,protocol,recv-q,send-q,local address,foreign address,state,pid/program name,path\n')
	if len(sock_csv.read())==0:
		sock_csv.write('status,date,protocol,refcnt,flags,type,state,i-node,pid/program name,path\n')
	...
	while 1:
		...
		for i in new_sock:
			sock_f.write(" [+]     {:20s} {}\n".format(time.strftime("%H:%M:%S %m/%d/%Y"), i))
			sock_csv.write('new,'+','.join(i.split())+'\n')
		for i in closed_sock:
			sock_f.write(" [-]     {:20s} {}\n".format(time.strftime("%H:%M:%S %m/%d/%Y"), i))
			sock_csv.write('closed,'+','.join(i.split())+'\n')
		conn=temp.conn
		sock=temp.sock
		os.popen('clear')
		time.sleep(30)
